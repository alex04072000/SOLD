...
def train():
    with tf.Graph().as_default():
        ...
        def PWC_full(B, lvl_h, lvl_w, pwc_h, pwc_w, lvl, frameNum=5, blur=False):
        ...
        def PWC_full_FB(F, B, lvl_h, lvl_w, pwc_h, pwc_w, lvl, frameNum=5):
            ratio_h = float(lvl_h) / float(pwc_h)
            ratio_w = float(lvl_w) / float(pwc_w)
            nn = ModelPWCNet(mode='test', options=nn_opts)
            nn.print_config()
        ...
        """clip to 0 1"""
        F_pred_0 = []
        F_pred_0.append(fused_frames[0]*tf.clip_by_value(A_pred_0[0], 0.0, 1.0))
        F_pred_0.append(fused_frames[1]*tf.clip_by_value(A_pred_0[1], 0.0, 1.0))
        F_pred_0.append(fused_frames[2]*tf.clip_by_value(A_pred_0[2], 0.0, 1.0))
        F_pred_0.append(fused_frames[3]*tf.clip_by_value(A_pred_0[3], 0.0, 1.0))
        F_pred_0.append(fused_frames[4]*tf.clip_by_value(A_pred_0[4], 0.0, 1.0))
        """erosion for PWC"""
        F_pred_0_erosion = []
        F_pred_0_erosion.append(fused_frames[0]*tf.clip_by_value(A_pred_0[0], 0.0, 1.0))
        F_pred_0_erosion.append(fused_frames[1]*tf.clip_by_value(A_pred_0[1], 0.0, 1.0))
        F_pred_0_erosion.append(fused_frames[2]*tf.clip_by_value(A_pred_0[2], 0.0, 1.0))
        F_pred_0_erosion.append(fused_frames[3]*tf.clip_by_value(A_pred_0[3], 0.0, 1.0))
        F_pred_0_erosion.append(fused_frames[4]*tf.clip_by_value(A_pred_0[4], 0.0, 1.0))
        for i in range(5):
            A_pred_0[i] = tf.clip_by_value(A_pred_0[i], 0.0, 1.0)
        ...
        kernel = tf.case([(tf.less(global_step, 50), lambda: generate_gaussian_kernel(11)),
                                    (tf.less(global_step, 100), lambda: generate_gaussian_kernel(11)),
                                    (tf.less(global_step, 150), lambda: generate_gaussian_kernel(11)),
                                    (tf.less(global_step, 200), lambda: generate_gaussian_kernel(11))],
                                   default=lambda: generate_gaussian_kernel(11), exclusive=False)
        kernel_small = tf.case([(tf.less(global_step, 50), lambda: generate_gaussian_kernel(33)),
                                    (tf.less(global_step, 100), lambda: generate_gaussian_kernel(33)),
                                    (tf.less(global_step, 150), lambda: generate_gaussian_kernel(33)),
                                    (tf.less(global_step, 200), lambda: generate_gaussian_kernel(33))],
                                   default=lambda: generate_gaussian_kernel(33), exclusive=False)
        ...
        def apply_small_gaussian_blur_image(x):
            x = tf.pad(x, [[0, 0], [40, 40], [40, 40], [0, 0]], 'SYMMETRIC')
            x_0 = tf.nn.conv2d(x[..., 0:1], kernel_small, strides=[1, 1, 1, 1], padding="SAME")
            x_1 = tf.nn.conv2d(x[..., 1:2], kernel_small, strides=[1, 1, 1, 1], padding="SAME")
            x_2 = tf.nn.conv2d(x[..., 2:3], kernel_small, strides=[1, 1, 1, 1], padding="SAME")
            output = tf.concat([x_0, x_1, x_2], -1)
            return output[:, 40:-40, 40:-40]
        def apply_gaussian_blur_flow(x):
            x = tf.pad(x, [[0, 0], [40, 40], [40, 40], [0, 0]], 'SYMMETRIC')
            x_0 = tf.nn.conv2d(x[..., 0:1], kernel_small, strides=[1, 1, 1, 1], padding="SAME")
            x_1 = tf.nn.conv2d(x[..., 1:2], kernel_small, strides=[1, 1, 1, 1], padding="SAME")
            output = tf.concat([x_0, x_1], -1)
            return output[:, 40:-40, 40:-40]
        F_max = tf.reduce_max(tf.concat([F_pred_0_erosion[0], F_pred_0_erosion[1], F_pred_0_erosion[2], F_pred_0_erosion[3], F_pred_0_erosion[4]], -1), keepdims=True)
        F_min = tf.reduce_min(tf.concat([F_pred_0_erosion[0], F_pred_0_erosion[1], F_pred_0_erosion[2], F_pred_0_erosion[3], F_pred_0_erosion[4]], -1), keepdims=True)
        ...
        for i in range(len(F_pred_0_erosion)):
            tmp_F = apply_gaussian_blur_image(F_pred_0_erosion[i])
            tmp_B = apply_small_gaussian_blur_image(B_pred_0[i])
            F_blurred.append((tmp_F - F_min) / (F_max - F_min + 1e-6))
            B_blurred.append((tmp_B - B_min) / (B_max - B_min + 1e-6))

        FF_full, FB_full = PWC_full_FB(F_blurred, B_blurred,
        ...
        A_pred = []
        A_pred.append(tf.concat([A_pred_0[0], A_pred_0[1], A_pred_0[2], A_pred_0[3], A_pred_0[4]], -1))
        """finest level flow"""
        FF_ = []
        FB_ = []
        for i in range(len(FF_full)):
            FF_sub = []
            FB_sub = []
            for j in range(len(FF_full[i])):
                FB_sub.append(FB_full[i][j])
                FF_sub.append(apply_gaussian_blur_flow(FF_full[i][j]))
        ...
        for i in range(1):
            ...
            def compute_loss_2(FF02, FB02, FF12, FB12, FF32, FB32, FF42, FB42, F2, B2, A2, I2_lvl, I0_lvl, I1_lvl, I3_lvl,
                               I4_lvl):
                sub_loss = 0
                ...
                sub_loss += (loss_weight[i] * tf.reduce_sum(tf.abs(I0_lvl - warp(F2, FF02, CROP_PATCH_H, CROP_PATCH_W) - warp(B2, FB02, CROP_PATCH_H, CROP_PATCH_W)*warp_1c(1.0-A2, FF02, CROP_PATCH_H, CROP_PATCH_W)) * outmask) / (3*tf.reduce_sum(outmask)+1e-10))
                outmask = create_outgoing_mask(FF12) * create_outgoing_mask(FB12)
                sub_loss += (loss_weight[i] * tf.reduce_sum(tf.abs(I1_lvl - warp(F2, FF12, CROP_PATCH_H, CROP_PATCH_W) - warp(B2, FB12, CROP_PATCH_H, CROP_PATCH_W)*warp_1c(1.0-A2, FF12, CROP_PATCH_H, CROP_PATCH_W)) * outmask) / (3*tf.reduce_sum(outmask)+1e-10))
                ...
                sub_loss += (loss_weight[i] * tf.reduce_sum(tf.abs(I3_lvl - warp(F2, FF32, CROP_PATCH_H, CROP_PATCH_W) - warp(B2, FB32, CROP_PATCH_H, CROP_PATCH_W)*warp_1c(1.0-A2, FF32, CROP_PATCH_H, CROP_PATCH_W)) * outmask) / (3*tf.reduce_sum(outmask)+1e-10))
                outmask = create_outgoing_mask(FF42) * create_outgoing_mask(FB42)
                sub_loss += (loss_weight[i] * tf.reduce_sum(tf.abs(I4_lvl - warp(F2, FF42, CROP_PATCH_H, CROP_PATCH_W) - warp(B2, FB42, CROP_PATCH_H, CROP_PATCH_W)*warp_1c(1.0-A2, FF42, CROP_PATCH_H, CROP_PATCH_W)) * outmask) / (3*tf.reduce_sum(outmask)+1e-10))

                """TV loss"""
                sub_loss += (loss_weight[i] * (1.0 * tf.reduce_mean(tf.abs(A2[:, 1:] - A2[:, :-1]))))
                sub_loss += (loss_weight[i] * (1.0 * tf.reduce_mean(tf.abs(A2[:, :, 1:] - A2[:, :, :-1]))))
                sub_loss += (loss_weight[i] * (0.01 * tf.reduce_mean(tf.abs(B2[:, 1:] - B2[:, :-1]))))
                sub_loss += (loss_weight[i] * (0.01 * tf.reduce_mean(tf.abs(B2[:, :, 1:] - B2[:, :, :-1]))))
                return sub_loss
            ...
            loss += compute_loss_2(FF_[0][2], FB_[0][2], FF_[1][2], FB_[1][2], FF_[3][2], FB_[3][2], FF_[4][2], FB_[4][2],
                                   ...
                                   tf.clip_by_value(A_pred[i][..., 2:3], 0.0, 1.0),
            ...
            loss += compute_loss_2(FF_[1][0], FB_[1][0], FF_[2][0], FB_[2][0], FF_[3][0], FB_[3][0], FF_[4][0], FB_[4][0],
                                   ...
                                   tf.clip_by_value(A_pred[i][..., 0:1], 0.0, 1.0),
            ...
            loss += compute_loss_2(FF_[0][1], FB_[0][1], FF_[2][1], FB_[2][1], FF_[3][1], FB_[3][1], FF_[4][1], FB_[4][1],
                                   ...
                                   tf.clip_by_value(A_pred[i][..., 1:2], 0.0, 1.0),
            ...
            loss += compute_loss_2(FF_[0][3], FB_[0][3], FF_[1][3], FB_[1][3], FF_[2][3], FB_[2][3], FF_[4][3], FB_[4][3],
                                   ...
                                   tf.clip_by_value(A_pred[i][..., 3:4], 0.0, 1.0),
            ...
            loss += compute_loss_2(FF_[0][4], FB_[0][4], FF_[1][4], FB_[1][4], FF_[2][4], FB_[2][4], FF_[3][4], FB_[3][4],
                                   ...
                                   tf.clip_by_value(A_pred[i][..., 4:5], 0.0, 1.0),
        ...
        tf.summary.image('sum0to2', tf.clip_by_value(warp(F_pred_0[2], FF_[0][2], CROP_PATCH_H, CROP_PATCH_W) + warp(B_pred_0[2], FB_[0][2], CROP_PATCH_H, CROP_PATCH_W)*warp_1c((1.0-A_pred_0[2]), FF_[0][2], CROP_PATCH_H, CROP_PATCH_W), 0.0, 1.0), 3)
        ...
        tf.summary.image('A2_pred_0', A_pred_0[2], 3)
        tf.summary.image('A0_pred_0', A_pred_0[0], 3)
        ...
        saver5.restore(sess, 'train_dir_imgReconstruction_Fence/model.ckpt-239999')
